...
public class MavenWtpPreferencePage extends PropertyPage implements IWorkbenchPreferencePage {
  ...
  private void updateImpactedProjects() {
    ...
    final MavenPlugin mvnPlugin = MavenPlugin.getDefault();
    final MavenProjectManager projectManager = mvnPlugin.getMavenProjectManager();
    ...
    WorkspaceJob job = new WorkspaceJob("Updating maven projects ") {
      ...
      public IStatus runInWorkspace(IProgressMonitor monitor) {
        try {
          ...
          final IMavenConfiguration mavenConfiguration = mvnPlugin.getMavenConfiguration();
          final IProjectConfigurationManager configurationManager = mvnPlugin.getProjectConfigurationManager();
          ...
          for(IMavenProjectFacade facade : facades) {
            ...
            configurationManager.updateProjectConfiguration(project, configuration,
                mavenConfiguration.getGoalOnUpdate(), subProgress);
          }
    ...
    job.setRule(mvnPlugin.getProjectConfigurationManager().getRule());
    job.schedule();
  }
   ...
   * @return
   */
  private List<IMavenProjectFacade> getImpactedProjects(final MavenProjectManager projectManager) {
